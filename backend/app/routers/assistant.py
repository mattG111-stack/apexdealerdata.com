...
class AskOut(BaseModel):
    answer: str
    # Optional chart spec drawn by the ask page — always the same data the
    # text states, never a separate computation.
...
def ask_question(
    ...
    # Deterministic layer first — Jarvis's way. The common questions answer
    # instantly from SQL, cost nothing, don't touch the monthly quota, and
    # can't be wrong the way a model can. Claude only sees the long tail.
    from ..assistant.rules import try_answer
    from ..assistant.sql import Scope
    from ..scoping import scope_params

    dealer_ids, admin_flag = scope_params(db, me)
    ruled = try_answer(body.question, Scope(dealer_ids, admin_flag), db)
    if ruled is not None:
        return AskOut(
            answer=ruled["text"],
            chart=ruled.get("chart"),
            tool_calls=[ToolCallOut(name="direct")],
    ...
    try:
        ...
        raise HTTPException(
            ...
            "I can answer that once the Claude key is added in admin settings. "
            "Meanwhile try: the plays, sales last week, stock over 90 days, "
            "fastest movers, or what's priced furthest from market.",
        ) from None
