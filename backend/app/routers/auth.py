...
def _send_email_code(db: Session, u: User) -> None:
    ...
    send_email(u.email, "Your Apex verification code",
               verification_email_html(code, settings.verify_code_ttl_minutes),
               text=f"Your Apex verification code is {code} (valid {settings.verify_code_ttl_minutes} min).")


def _send_phone_code(db: Session, u: User) -> None:
    code = issue_code(db, u.id, "phone")
    send_sms(u.phone or "", f"Your Apex code is {code}")
...
@admin_router.delete("/{user_id}", status_code=204, response_class=Response,
                     response_model=None)
def delete_user(
    user_id: int,
    ...
    Their dealer grants and verification codes go with them. An admin cannot
    delete themselves — locking the last admin out of the platform is not an
    action anyone means to take.
    """
    if user_id == me.id:
        raise HTTPException(
    ...
    from ..models import DealerAccess, VerificationCode

    db.query(DealerAccess).filter(DealerAccess.user_id == user_id).delete()
