...
1. **Every weekly snapshot is kept, and listings are snapshot-scoped.** Ollie kept
   one active batch and archived the rest, because a property valuation only cares
   about today. Apex cares about the *trajectory* — what a car was first asked at,
...
class SnapshotStatus(str, Enum):
    """Two-stage publish, carried over from Ollie: load, review, then go live."""
    STAGED = "staged"
    PUBLISHED = "published"
    ARCHIVED = "archived"
...
class WeeklySnapshot(Base):
    """One weekly listings file. Unlike Ollie's ImportBatch, these are never
...
class Listing(_VehicleMixin, Base):
    ...
    # is how Ollie worked: the API serves stored values, so changing the pricing
...
class AppSetting(Base):
    ...
    Ollie asked each user for their own LLM key, which suited an investor tool.
