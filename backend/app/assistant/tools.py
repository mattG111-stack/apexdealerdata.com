...
def my_sales_vs_market(weeks: int, scope: "Scope | None") -> str:
    ...
    return _q(
        ...
        -- A "sold week" derived after a snapshot gap holds several weeks of
        -- disappearances. weeks_covered says how many, so counts can be read
        -- as a rate instead of a spike.
        gaps AS (
            SELECT week_ending,
                   GREATEST(1, ROUND((week_ending - LAG(week_ending)
                       OVER (ORDER BY week_ending)) / 7.0)) AS weeks_covered
        ...
        SELECT m.sold_week,
               ...
               COALESCE(g.weeks_covered, 1) AS weeks_covered
        FROM market_sales m
        ...
        GROUP BY m.sold_week, g.weeks_covered
