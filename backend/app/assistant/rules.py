...
def _sales_last_week(q: str, scope: Scope, db: Session) -> str | None:
    ...
    for r in rows:
        lines.append(
            ...
            f" ({r['market_avg_days']} days avg)"
        )
    if len(rows) >= 2:
        now, prev = rows[0]["my_sales"] or 0, rows[1]["my_sales"] or 0
        if prev:
            diff = now - prev
            word = "up" if diff > 0 else "down" if diff < 0 else "flat"
            lines += ["", f"That's {word} {abs(diff)} on the week before."]
    lines += ["", "Newest week is provisional — about 4% of derived sales turn out to be relists."]
    chart = {
        "type": "bar",
        "title": "Your sales by week",
        "points": [
            {"label": r["sold_week"][5:], "value": r["my_sales"] or 0}
