...
_LISTED_SQL = text(
    ...
    SELECT make, model, spec AS variant, year, kms, price,
           ...
           number_of_days_listed AS days_listed, transmission, ext_color,
...
def price_vehicle(db: Session, vehicle: dict) -> dict:
    ...
    return {
        ...
        # Rows for a comps table — what the old system showed, plus days and
        # whether the car actually sold rather than merely asked.
        "comp_rows": [
            {
                "year": c.get("year"), "variant": c.get("variant"),
                "region": c.get("region"), "kms": c.get("kms"),
                "price": c.get("price"), "days": c.get("days_listed"),
                "fuel": c.get("fuel_type"), "transmission": c.get("transmission"),
                "colour": c.get("ext_color"),
                "status": "SOLD" if c.get("src") == "sold" else "for sale",
            }
            for c in sorted(found.comps, key=lambda x: x.get("km_diff") or 0)[:15]
        ],
        "comp_points": [
            {
                "kms": c.get("kms"),
